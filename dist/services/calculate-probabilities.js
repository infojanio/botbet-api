...
exports.betaPosteriorMean = betaPosteriorMean;
exports.freqOver = freqOver;
exports.calcOverProb = calcOverProb;
function betaPosteriorMean(successes, trials, p0 = 0.55, n0 = 10) {
    const alpha = p0 * n0;
    const beta = (1 - p0) * n0;
    return (successes + alpha) / (trials + alpha + beta);
}
function freqOver(values, line) {
    const n = values.length;
    const s = values.filter(v => v > line).length;
    return { n, s, pHat: n ? s / n : 0 };
}
function calcOverProb(h2h, recent, line, leagueAvg = 0.55) {
